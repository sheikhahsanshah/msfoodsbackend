...
export const orderController = {
  ...
  createOrder: async (req, res) => {
    ...
    try {
      ...
      if (couponCode) {
        if (!req.user) {
          ...
          return handleError(
            res,
            401,
            "Authentication required for coupon use"
          );
        }
        try {
      ...
      const order = new Order({
        ...
        paymentStatus:
          paymentMethod === "BankTransfer" ? "Pending" : "Confirmed",
  ...
  handlePayfastNotification: async (req, res) => {
    try {
      ...
      order.status =
        data.payment_status === "COMPLETE" ? "Processing" : "Cancelled";
      await order.save();
  ...
  for (const item of items) {
    ...
    // Priority 2: Global sale percentage (if no individual sale price)
      ...
      const calculatedSalePrice = Math.round(
        priceOption.price * (1 - product.sale / 100)
      );
      // Only apply if it's a meaningful discount (at least 1%)
      if (
        calculatedSalePrice < priceOption.price &&
        (priceOption.price - calculatedSalePrice) / priceOption.price >= 0.01
      ) {
...
const validateCoupon = async (code, userId, subtotal, orderItems, session) => {
  ...
  console.log("Coupon validation debug:", {
    ...
    maxPurchase: coupon.maxPurchase
      ? `${eligibleSubtotal <= coupon.maxPurchase} (${eligibleSubtotal} <= ${coupon.maxPurchase})`
      : "No max",
    eligibleUser:
      !coupon.eligibleUsers?.length ||
      coupon.eligibleUsers.some((u) => u._id.equals(userId)),
  ...
  try {
    ...
    const statusParams = {
      Processing: {
        ...
        params: {
          estimated_date: formatDateForTemplate(
    ...
    if (statusParams[status]) {
      ...
      if (contactInfo.email) {
        ...
        try {
          ...
          console.error(
            "❌ Failed to send status email to customer:",
            contactInfo.email,
            error
          );
        }
      ...
      if (process.env.EMAIL_DEFAULT) {
        try {
          console.log(
            "Sending status email to default address:",
          ...
          console.log(
            "✅ Status email sent to default address:",
            process.env.EMAIL_DEFAULT
          );
        } catch (error) {
          console.error(
            "❌ Failed to send status email to default address:",
            process.env.EMAIL_DEFAULT,
            error
          );
        }
